                                        ...
                                        gpointer        data)
{
    ...
    if (path && gtk_tree_selection_path_is_selected (selection, path))
    {
        ...
        priv_data->motion_notify_handler =
            g_signal_connect (tree_view, "motion_notify_event", G_CALLBACK (egg_tree_multi_drag_motion_event), NULL);
        priv_data->button_release_handler =
            g_signal_connect (tree_view, "button_release_event", G_CALLBACK (egg_tree_multi_drag_button_release_event), NULL);

        if (priv_data->drag_data_get_handler == 0)
        {
            priv_data->drag_data_get_handler =
                g_signal_connect (tree_view, "drag_data_get", G_CALLBACK (egg_tree_multi_drag_drag_data_get), NULL);
        }
...
egg_tree_multi_drag_add_drag_support (GtkTreeView *tree_view)
{
    ...
    g_signal_connect (tree_view, "button_press_event", G_CALLBACK (egg_tree_multi_drag_button_press_event), NULL);
}
